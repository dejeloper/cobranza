...
class Clientes_model extends CI_Model {
    ...
    public function obtenerNomClientesDir($EstExcep1 = 0, $EstExcep2 = 0, $EstExcep3 = 0) {
        $this->db->select('c.Codigo, c.Nombre, c.Telefono1, c.Telefono2, c.Telefono3, td.Nombre as TipDocNombre, e.Nombre as EstNombre, d.Direccion as Dir, d.Etapa, d.Torre, d.Apartamento, d.Manzana, d.Interior, d.Casa, d.Barrio, p.DiaCobro');
        $this->db->from('Clientes as c');
        $this->db->join('TiposDocumentos as td', 'c.TipoDocumento = td.Codigo');
        $this->db->join('Estados as e', 'c.Estado = e.Codigo');
        $this->db->join('Direcciones as d', 'c.Direccion = d.Codigo');
        ...
        $this->db->where('c.Habilitado', '1');
        $this->db->order_by('p.DiaCobro', 'ASC');
        $query = $this->db->get();
        //echo $this->db->last_query() . "<br><br>";
        if ($query->num_rows() <= 0) {
            return false;
