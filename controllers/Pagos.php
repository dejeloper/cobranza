            ...
            if (isset($dataPedido) && $dataPedido != FALSE) {
                ...
                foreach ($dataPedido as $item1) {
                    ...
                    if (array_key_exists($i, $dataPagoPedido)) {
                        ...
                        $dataPagosP["saldo"] = $item1["SaldoActual"];//intval($item1["Valor"]) - intval($abonado);
...
//        $num = $dataPagos[0]["Cuotas"];
                    ...
                    if (isset($dataPedido) && $dataPedido != FALSE) {
                        ...
                        foreach ($dataPedido as $item1) {
                            ...
                            if (array_key_exists($i, $dataPagoPedido)) {
                                ...
                                $dataPagosP["abonado"] = $abonado;
                                $dataPagosP["valor"] = $item1["Valor"];
                                $dataPagosP["saldo"] = intval($item1["SaldoActual"]);
                ...
                if ($pedido != null && $pedido != "") {
                    ...
                    if (isset($dataProgramados) && $dataProgramados != FALSE) {
                        ...
                        if ($copias != FALSE) {
                            ...
                            if ($sub == "Plataforma") {
                                $html = $this->RecibosSoloInfoAdriana($dataProgramados[0], $dataCliente[0], $dataAdmin, $pagina, $margen);
                             } elseif ($sub == "adriana") {
                                $html = $this->RecibosSoloInfoAdriana($dataProgramados[0], $dataCliente[0], $dataAdmin, $pagina, $margen);
                            } else {
    ...
    public function ImprimirRecibos($margen) {
        ...
        if ($data != null) {
            ...
            foreach ($data as $value) {
                ...
                if ($copias != FALSE) {
                    if ($numRecibos > 0) {
                        //$saltoNuevoRecibo = "<br><br><br>";
                        //$saltoNuevoRecibo = "";
                        if ($sub == "adriana") { 
                            echo "<p style='margin-bottom: -5px;'>.</p>";
                        } elseif ($sub == "Plataforma") {
                            echo "<p style='margin-bottom: -5px;'>.</p>";
                        }
                    ...
                    $html = "";
                    if ($sub == "adriana") {
                        $html = $this->RecibosSoloInfoAdrianaMultiple($data[$i], $dataCliente[0], $dataAdmin, $pagina, $margen);
                    } elseif ($sub == "Plataforma") {
                        $html = $this->RecibosSoloInfoAdrianaMultiple($data[$i], $dataCliente[0], $dataAdmin, $pagina, $margen);
                    } else {
                        echo '<script>alert("No tiene un formato configurado. Por favor comuniquese con su administrador");  window.open("","_parent",""); window.close(); </script>';
    ...
    public function RecibosSoloInfoAdriana($data, $cliente, $dataAdmin, $pagina, $margen) {
        /*var_dump($data);
        echo "<br><br>";
        var_dump($cliente);
        echo "<br><br>";
        ...
        $fecha = strftime("%d/%m/%Y", strtotime($data["FechaProgramada"])) . ' - ' . strftime("%d/%m/%Y", strtotime($data["FechaProgramada"] . "+ 8 days"));
        $nombreCliente = $cliente["Nombre"];
        $paginaFisica = $data["Pagina"];
        $direccionCliente = $cliente["Dir"] . " Bar: " . $cliente["Barrio"];
        $etapa = $cliente["Etapa"];
        if ($etapa != ""){
            $direccionCliente = $direccionCliente . " Eta: " . $etapa;
        } 
        $torre = $cliente["Torre"];
        if ($torre != "" ) {
            $direccionCliente = $direccionCliente . " To: " . $torre;
        }
        $apartamento = $cliente["Apartamento"];
        if ($apartamento != "" ) {
            $direccionCliente = $direccionCliente . " Apto: " . $apartamento;
        }
        $manzana = $cliente["Manzana"];
        if ($manzana != "" ) {
            $direccionCliente = $direccionCliente . " Conj: " . $manzana;
        }
        $interior = $cliente["Interior"];
        if ($interior != "" ) {
            $direccionCliente = $direccionCliente . " Int: " . $interior;
        }
        $casa = $cliente["Casa"];
        if ($casa != "" ) {
            $direccionCliente = $direccionCliente . " Cs: " . $casa;
        }
        ...
        //echo $formatterES->format($abono);
        $abono_letras = "...";
        if ($data["Valor"] == 50000){
            $abono_letras = "Cincuenta mil Pesos";
        }
        if ($data["Valor"] == 100000){
            $abono_letras = "Cien mil Pesos";
        }
        if ($data["Valor"] == 150000){
            $abono_letras = "Ciento cincuenta mil Pesos";
        }
        if ($data["Valor"] == 160000){
            $abono_letras = "Ciento sesenta mil Pesos";
        }
        if ($data["Valor"] == 200000){
            $abono_letras = "Doscientos mil Pesos";
        } 
        if ($data["Valor"] == 300000){
            $abono_letras = "Trescientos mil Pesos";
        } 
        if ($data["Valor"] == 400000){
            $abono_letras = "Cuatrocientos mil Pesos";
        } 
        ...
        $html = "
            ...
            <br />
            <br />
            <div style='width: 580px;" . $margen . "'>
                <br />
                <p style='margin-left: 320px; margin-top: -60px;' class='letraPeque'>" . $paginaFisica . "</p>
                <p style='margin-left: 70px; margin-top: 10px; display: block; float: left;'>" . $fecha . "</p>
                <p style='margin-left: 130px; margin-top: 10px; display: block; float: left;'>" . $cliente["Telefono1"] . '-' . $cliente["Telefono2"] . "</p>
                <p style='margin-left: 90px; margin-top: 60px; '>" . $nombreCliente . "</p>
                <p style='margin-left: 100px; margin-top: 30px; width: 400px; display: block; float: left;' class='letraPeque'>" . $direccionCliente . "</p>
                <br />
                <br /> 
                <br />
                <p style='margin-left: 80px; margin-top: 40px; display: block; float: left;'>" . $valor . "</p>
                <p style='margin-left: 100px; width: 320px; margin-top: 40px; display: block; float: left;'>" . $abono_letras . "</p>
                <p style='margin-left: 80px; margin-top: 0px; display: block; float: left;'>" . $abono . "</p>
                <p style='margin-left: 300px; margin-top: 0px; display: block; float: left;'>" . $cuota . "</p> 
                <p style='margin-left: 80px; margin-top: 0px; display: block; float: left; min-width: 70px;'>" . $saldo . "</p>
                <br />
                <br /> 
                <p style='margin-left: 150px; margin-top: 10px; display: block; float: left; width: 400px;' class='letraPeque'>" . $observaciones . "</p>                              
            </div>
            </section>
            ";
     ...
     public function RecibosSoloInfoAdrianaMultiple($data, $cliente, $dataAdmin, $pagina, $margen) {
        /*var_dump($data);
        echo "<br><br>";
        var_dump($cliente);
        echo "<br><br>";
        ...
        $html = "
            ...
            <div style='width: 580px;" . $margen . "'>
                <br />
                <p style='margin-left: 320px; margin-top: -60px;' class='letraPeque'>" . $paginaFisica . "</p>
                <p style='margin-left: 70px; margin-top: 16px; display: block; float: left;'>" . $fecha . "</p>
                <p style='margin-left: 130px; margin-top: 16px; display: block; float: left;'>" . $cliente["Telefono1"] . '-' . $cliente["Telefono2"] . "</p>
                <p style='margin-left: 90px; margin-top: 69px; '>" . $nombreCliente . "</p>
                <p style='margin-left: 100px; margin-top: 16px; width: 400px; display: block; float: left;' class='letraPeque'>" . $direccionCliente . "</p>
                <br />
                <br /> 
                <br />
                <p style='margin-left: 80px; margin-top: 40px; display: block; float: left;'>" . $valor . "</p>
                <p style='margin-left: 100px; width: 320px; margin-top: 40px; display: block; float: left;'>" . $abono_letras . "</p>
                <p style='margin-left: 80px; margin-top: -3px; display: block; float: left;'>" . $abono . "</p>
                <p style='margin-left: 300px; margin-top: -3px; display: block; float: left;'>" . $cuota . "</p> 
                <p style='margin-left: 80px; margin-top: 0px; display: block; float: left; min-width: 70px;'>" . $saldo . "</p>
                <br />
                <br /> 
                <p style='margin-left: 150px; margin-top: 7px; display: block; float: left; width: 400px;' class='letraPeque'>" . $observaciones . "</p>                              
            </div>
            </section>
            ";
